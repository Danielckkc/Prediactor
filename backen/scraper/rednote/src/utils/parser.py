...
def parse_comment_items(payload: Dict[str, Any], note_id: str) -> List[Dict[str, Any]]:
    ...
    for raw in comments:
        ...
        replies = raw.get("sub_comments") or raw.get("replies") or raw.get("subComments") or []
...
def parse_profile(payload: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    # The real user/otherinfo endpoint returns interactions as a LIST of
    # {type, name, count} entries; normalize it to a dict keyed by type.
    ...
    followers = (
        interactions.get("fans")
        or interactions.get("followers")
        or interactions.get("followerCount")
        or 0
    )
    likes = (
        interactions.get("interaction")  # "获赞与收藏" on user/otherinfo
        or interactions.get("likes")
